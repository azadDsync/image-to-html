...
export default function Home() {
  ...
  const generateHtmlDocument = ({
    ...
    const css = `
      ...
      .meta { margin-bottom: 16px; opacity: .8; font-size: 14px; }
      .grid {
        display: grid;
      ...
      .cell { aspect-ratio: 1 / 1; border-radius: 2px; }
    `;

    const cells = grid
      .map(({ row, col, color }) => {
        const rgb = `rgb(${color.r}, ${color.g}, ${color.b})`;
        return `<div class="cell" title="r${row} c${col} • ${rgb}" style="background:${rgb}"></div>`;
      })
      .join("\n");

    const title = `Image to Grid (${rows}x${cols})`;
...
<html lang="en">
  ...
  <body>
    <div class="wrap">
      <h1>${title}</h1>
      <div class="meta">Generated by image-to-html • ${new Date().toISOString()}</div>
      <div class="grid" role="img" aria-label="Pixel grid with ${rows} rows and ${cols} columns">
        ${cells}
      </div>
    </div>
  </body>
  ...
  const handleDownloadHtml = () => {
    ...
    const html = generateHtmlDocument({ grid: data, rows, cols });
    downloadTextFile({
      filename: `grid-${rows}x${cols}.html`,
      text: html,
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4 flex-1 justify-end">
              ...
              <button
                ...
                disabled={!data || data.length === 0}
                className="bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 disabled:bg-gray-300 dark:disabled:bg-gray-700 text-white font-medium py-2 px-4 rounded-lg transition-all duration-200 text-sm shadow-sm hover:shadow-md disabled:cursor-not-allowed whitespace-nowrap disabled:opacity-50"
                aria-label="Download generated grid as an HTML file"
              >
                ⬇️ Download HTML
              </button>
